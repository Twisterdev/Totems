...
@synthesize ropeSprites = _ropeSprites;
...
-(void)createRope:(CGPoint)pointA pointB:(CGPoint)pointB {
	...
	self.ropeSprites = [[NSMutableArray alloc] init];
	...
	if(spriteSheet!=nil) {
		for(int i=0;i<numPoints-1;i++) {
			...
			[_ropeSprites addObject:tmpSprite];
		}
	}
}
...
-(void)removeSprites {
	for(int i=0;i<numPoints-1;i++) {
		CCSprite *tmpSprite = [_ropeSprites objectAtIndex:i];
		[spriteSheet removeChild:tmpSprite cleanup:YES];
	}
	[_ropeSprites removeAllObjects];
	[_ropeSprites release];
}
