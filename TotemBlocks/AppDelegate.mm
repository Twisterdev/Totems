...
//		CCLOG(@"Retina Display Not supported");
    ...
    Level *level1 = [[[Level alloc] initWithLevelNum:1 bgImage:@""] autorelease];
    
    // Puefo agregar muchos niveles más adelante
    //Level *level2 = [[[Level alloc] initWithLeveImg:@"bg2.png"] autorelease];
    [_levels addObject:level1];
    // Si hay mas niveles hay distintas propiedades que setiarles y agreagrlas al array
    //[_levels addObject:level2];
    self.curLevelIndex = 0;
    
    // Recive una escena
    self.mainScene = [HelloWorldScene scene];
    // Recive un objeto de tipo AddLevel que hereda de CCSene por lo tanto tiene metodo node.
    //self.newLevelScene = [AddLevel node];
